...
export function SchoolUsersListPage({ config }: { config: UsersModuleConfig }) {
  const { usersBasePath, studentsHref, assignableRoles } = config;
  ...
  const roleOptions = useMemo(
    () => [
      ...
      ...assignableRoles.map((r) => ({ value: r, label: roleLabel(r) })),
    ],
    [assignableRoles],
  );
  ...
  return (
    <PageWrapper title="Users" description="Staff accounts and school enrollment overview">
      <div className="mb-8">
        <StudentEnrollmentCharts studentsHref={studentsHref} />
      </div>
      ...
      <div className="mb-4 flex flex-col justify-between gap-3 lg:flex-row lg:items-end">
        ...
        <Link href={`${usersBasePath}/create`}>
      ...
      {selected.length ? (
        <div className="mb-3 flex flex-wrap items-center gap-2">
          ...
          <Button variant="secondary" loading={busyBulk} onClick={() => void runBulk("activate")}>
            Bulk activate
          </Button>
          <Button variant="secondary" loading={busyBulk} onClick={() => void runBulk("deactivate")}>
            Bulk deactivate
          </Button>
          <Button loading={busyBulk} onClick={() => void runBulk("delete")}>
            Bulk delete
          </Button>
        </div>
      ) : null}
            ...
            <tbody className="divide-y divide-border bg-card">
              {loading ? (
                ...
                users.map((r) => (
                  <tr
                    key={r.id}
                    className="transition-ui odd:bg-background even:bg-muted/20 hover:bg-accent/60"
                  >
                    ...
                    <td className="px-3 py-1.5 align-middle font-medium">
                      <Link className="hover:underline" href={`${usersBasePath}/${r.id}`}>
                        {r.fullName}
                      </Link>
                    </td>
                    <td className="px-3 py-1.5 align-middle text-muted-foreground">{r.email}</td>
                    <td className="px-3 py-1.5 align-middle">
                      <Badge tone="success">{roleLabel(r.role)}</Badge>
                    </td>
                    <td className="px-3 py-1.5 align-middle">
                      <Badge tone={r.isActive ? "success" : "warning"}>{r.isActive ? "Active" : "Inactive"}</Badge>
                    </td>
                    <td className="px-3 py-1.5 align-middle text-muted-foreground">
                      {r.createdAt ? new Date(r.createdAt).toLocaleDateString() : "—"}
                    </td>
                    <td className="flex gap-1 px-3 py-1.5 align-middle">
