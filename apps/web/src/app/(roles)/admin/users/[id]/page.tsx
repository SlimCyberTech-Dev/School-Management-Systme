...
export default function AdminUserDetailPage() {
  ...
  return (
    <PageWrapper title="User Profile" description="Account details and audit history">
      <div className="mb-3"><Link href="/admin/users">← Back to Users</Link></div>
        ...
        <div className="space-y-4">
          <Card title={user.fullName}>
            <div className="mb-3 flex flex-wrap gap-2">
              <Badge tone="success">{user.role.replace(/_/g, " ")}</Badge>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {user.isActive ? (
                <Button loading={busy} disabled={user.systemAccount} onClick={() => void quickAction("deactivate")}>Deactivate</Button>
              ) : (
                <Button loading={busy} onClick={() => void quickAction("activate")}>Activate</Button>
              )}
              <Button variant="secondary" loading={busy} onClick={() => void quickAction("unlock")}>Unlock</Button>
              <Button variant="secondary" onClick={() => setShowReset(true)}>Reset Password</Button>
              <Link href={`/admin/users/${user.id}/edit`}><Button variant="secondary">Edit</Button></Link>
            </div>
          </Card>

          <div className="flex gap-2">
            <Button variant={tab === "details" ? "primary" : "secondary"} onClick={() => setTab("details")}>Account Details</Button>
            <Button variant={tab === "audit" ? "primary" : "secondary"} onClick={() => setTab("audit")}>Audit History</Button>
          </div>
          ...
          {tab === "details" ? (
            <Card title="Operational Details">
              ...
              <div className="mt-4 space-y-2">
                ...
                <Button loading={busy} onClick={() => void saveNotes()}>Save notes</Button>
              </div>
            </Card>
          ) : (
